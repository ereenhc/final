...
@media (max-width: 900px) {
    ...
    .menu a {
        ...
        .main-container { flex-grow: 1; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding: 40px; }
        h2 { text-align: center; margin-bottom: 20px; }
        #add-question-btn { background: #5cb85c; color: #fff; border: none; border-radius: 8px; font-size: 22px; padding: 18px 38px; cursor: pointer; margin-top: 30px; }
        #add-question-btn:hover { background: #409a40; }
        #question-form, #select-type { display: none; margin: 24px 0; background: #f2f6fa; padding: 25px; border-radius: 10px; box-shadow: 0 2px 12px rgba(61,131,184,0.06);}
        #question-form input[type="text"], #question-form textarea { width: 98%; padding: 10px; margin-bottom: 14px; font-size: 16px; border-radius: 6px; border: 1px solid #bbb; }
        .options-row { display: flex; gap: 8px; }
        .options-row input[type="text"] { width: 85%; }
        .answer-radio { margin-right: 6px; }
        .submit-btn { background: #ff8500; color: #fff; font-weight: bold; padding: 12px 38px; border-radius: 7px; font-size: 19px; border: none; cursor: pointer; }
        .submit-btn:hover { background: #e06309; }
        #quiz-list { margin-top: 32px; width: 100%; max-width: 600px; }
        .quiz-item { background: #fff; border-radius: 12px; margin-bottom: 24px; box-shadow: 0 2px 6px rgba(0,0,0,.10); padding: 22px 18px;}
        .option-btn { background: #4285f4; color: #fff; font-size: 17px; padding: 10px 20px; margin: 5px 7px 0 0; border-radius: 7px; border: none; cursor: pointer; min-width: 100px;}
        .option-btn .count { font-size: 15px; background: #fff; color: #4285f4; border-radius: 20px; padding: 2px 10px; margin-left: 8px;}
        .option-btn.selected { background: #333; }
    </style>
</head>
<body>
<div class="sidebar">
...
questionForm.onsubmit = function(e) {
    ...
    fetch('addQuiz.php', {method:'POST', body:data})
    .then(res => res.json())
    .then(resp => {
        if(resp.success) {
